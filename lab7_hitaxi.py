...
def find_index(s, t):
    ...
    If t is not present in s, the function returns -1.
    ...
    - t (string): The single character to search for.

    Returns:
    - (int): The first index where t is found, or -1 if not found.
...
def project_completion_day(day, days_to_completion):
    """
    Returns the day of the week a project will be finished, given the 
    starting day and the number of days it will take to complete.
    ...
    - days_to_completion (int): Number of days until the project is done.

    Returns:
    - (string): The day of the week the project finishes.
    """
...
# Your unit tests
line='2024-03-05 14:32:15 [ERROR] database.py Connection timeout after 30s'
expected= ('2024-03-05 14:32:15', 'ERROR', 'database.py', 'Connection timeout after 30s')
...
for log_line in log_string.split('\n'):
    if log_line.strip(): 
        result = parse_log_line(log_line)
        parsed_logs_list.append(result)


print(f"Parsed {len(parsed_logs_list)} logs into the list.")
